    ...
    return es.merge([
        ...
        gulp
            .src([
                ...
                './node_modules/purecss-ui-menu/dist/purecss-ui-menu-default.css'
            ], { allowEmpty: true })
            .pipe(concat('purecss-ui-default.css'))
            .pipe(ifCondition(isMinified, cleanCSS()))
            .pipe(gulp.dest('./wwwroot/css')),
        gulp
            .src([
                './node_modules/purecss-ui/dist/purecss-ui-dark.css',
                './node_modules/purecss-ui-menu/dist/purecss-ui-menu-dark.css'
            ], { allowEmpty: true })
            .pipe(concat('purecss-ui-dark.css'))
            .pipe(ifCondition(isMinified, cleanCSS()))
            .pipe(gulp.dest('./wwwroot/css'))
    ]).on('end', resolve);
}
...
function compileThemeStream(theme, isMinified) {
    ...
    return gulp
        ...
        .pipe(rename('purecss-ui-dialog-' + theme + '.css'))
        .pipe(ifCondition(isMinified, cleanCSS()))
        .pipe(gulp.dest('./wwwroot/css'));
}
    ...
    return es.merge(
        gulp
            .src([
                ...
                './node_modules/purecss-ui-menu/dist/purecss-ui-menu.js'
            ], { allowEmpty: true })
        ...
        gulp
            ...
            .pipe(ifCondition(isTranspiled, babel({ presets: ['@babel/env'] })))
            .pipe(concat('purecss-ui-dialog.js'))
            .pipe(ifCondition(isMinified, uglify()))
            .pipe(gulp.dest('./wwwroot/js'))
    ).on('end', resolve);
}
...
function buildDistCss(resolve) {
    ...
    compileThemes(themes, false)
        .then(() => {
            es.merge([
                gulp.src(['./wwwroot/css/purecss-ui-dialog-*.css'])
                    .pipe(gulp.dest('./dist')),
                gulp.src(['./wwwroot/css/purecss-ui-dialog-*.css'])
                    .pipe(cleanCSS())
